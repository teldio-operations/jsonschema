// Package ordered holds a map that keeps its insertion order.
//
...
var (
	_ jsonv2.MarshalerTo     = &Map[string, any]{}
	_ jsonv2.UnmarshalerFrom = &Map[string, any]{}
)
...
func New[K ~string, V any]() *Map[K, V] {
	return &Map[K, V]{pairs: make(map[K]*Pair[K, V])}
}
...
func (m *Map[K, V]) Set(key K, value V) (V, bool) {
	...
	pair = &Pair[K, V]{Key: key, Value: value, prev: m.newest}
	m.pairs[key] = pair
...
func (m *Map[K, V]) Oldest() *Pair[K, V] {
	if m == nil {
		return nil
	}
...
func (m *Map[K, V]) Newest() *Pair[K, V] {
	if m == nil {
		return nil
	}
