...
func keys(t *testing.T, m *Map[string, int]) []string {
	t.Helper()
...
func filled(t *testing.T, in ...string) *Map[string, int] {
	t.Helper()
...
func TestSetOnAnExistingKeyKeepsItsPosition(t *testing.T) {
	m := filled(t, "first", "second", "third")

	old, found := m.Set("first", 99)
	if !found {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if m.Len() != len(test.want) {
				t.Errorf("Len = %d, want %d", m.Len(), len(test.want))
			}
		})
	}
}
...
func TestUnmarshalRejectsADuplicateNameUnderV2(t *testing.T) {
	...
	err := jsonv2.Unmarshal([]byte(`{"a":1,"a":2}`), &m)
	if err == nil {
		t.Fatal("expected an error")
	}
}
